...
def test_catalog_success_failure_rolls_back_and_never_reports_success(monkeypatch, tmp_path):
    ...
    raw, curated = write_batch(cfg, symbols=["US.SPY"])
    sealed = plan(cfg, ["US.SPY"])
    real_update = Catalog.update_purge_operation

    def fail_success(self, plan_id, **kwargs):
        if kwargs.get("state") == "SUCCESS":
            raise RuntimeError("simulated catalog transaction failure")
        return real_update(self, plan_id, **kwargs)

    monkeypatch.setattr(Catalog, "update_purge_operation", fail_success)
...
def test_completed_result_tampering_blocks_idempotent_retry(tmp_path):
    ...
    payload["moved_files"] = []
    Path(result.result_file).write_text(json.dumps(payload), encoding="utf-8")
