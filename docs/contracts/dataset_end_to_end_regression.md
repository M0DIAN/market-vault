...
| E2E_ROW_COLUMN_ORDER | materialization; reader | Semantically identical input permutations produce identical rows / content ID / `dataset_id`; the physical Parquet keeps `code`, `feature_window_close`, `sample_key` ASC and the exact authoritative column order; reordered rows, reordered columns, and dtype changes are rejected by the reader at the deep Parquet contracts (see below). |
| E2E_STAGING_RESIDUE | materialization | Any staging residue under `.staging-<dataset_id>` (empty, partial, missing `_SUCCESS`, corrupt `_SUCCESS`, or a complete-but-uncommitted copy) fails closed, is never adopted, never repaired, never deleted, and never published; a foreign staging directory never blocks the materializer. The complete-uncommitted case copies every artifact of a formally valid final Dataset (produced by the public chain through a separate output root, `_SUCCESS` included) into the staging path and asserts the residue's hashes, sizes, mtimes, and entry set stay untouched. |
...
| E2E_CORRUPTED_PARQUET | reader | Arbitrary bytes, one changed logical value, reordered rows, reordered columns, dtype changes, and metadata changes are all rejected; nothing is written, repaired, or regenerated; the original failure is preserved through the documented error chain (including the underlying `pa.ArrowException`); no partial `VerifiedDatasetBuild` is returned. The tamper tests synchronize the manifest `DatasetOutputFile` byte facts (size/SHA-256) with the rewritten Parquet so the reader passes the outer record checks and each test asserts the specific deep contract it reaches: the recomputed `logical_dataset_content_id`, the physical row order (`code`/`feature_window_close`/`sample_key` ASC), the authoritative Arrow field order/types/nullability, or the exact metadata key set. |
...
| E2E_NON_FINITE | Feature/Label execution; reader | A Feature transform returning NaN / +Inf / -Inf fails the build before anything is published. A Label implementation returning NaN / +Inf / -Inf under the fixed built-in `forward_return` transform_ref is really invoked by the production Label executor (test seam: the implementation temporarily takes the built-in module/name identity, the loaded `forward_return` module attribute is patched to it, and the registry is the built-in registry seam) and the production output validation rejects it; a transform_ref outside the fixed catalog additionally fails closed at the catalog boundary. A tampered Parquet carrying NaN is rejected by the reader (NaN is never silently null). |
| E2E_TIMEZONE_DST | split; identity; reader | Equivalent timezone representations of one absolute instant share identity; a real process-timezone switch (POSIX `time.tzset()` with `TZ=Asia/Tokyo`, restored in `finally`) never changes results — platforms without `time.tzset` (e.g. Windows) skip with the explicit environment reason while POSIX CI executes it; the materialized Parquet timing columns are `pa.timestamp("us", tz="UTC")` with read-back values equal to the expected UTC instants exactly; naive datetimes fail closed; spring-forward / fall-back boundaries use the declared IANA timezone's next local midnight (never fixed +24h); nominal split uses the declared local date; invalid IANA names fail closed. |
