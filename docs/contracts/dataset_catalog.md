...
exactly one match, and returns the exact existing
`DatasetCatalogSnapshotEntryRecord` object. `CONTENT_ID_MODEL=MODEL_A`:
`content_id` remains a verified entry fact and is not a selector.
...
snapshot identity, artifact bytes, or migration behavior.
