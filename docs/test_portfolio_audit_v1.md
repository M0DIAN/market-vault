...
- The runner-minute saving is real: the FULL step's 263.36s of pytest is removed; the duplicate targeted/frozen executions inside it disappear with it, while the standalone A/B prelude (0.87s + 35.10s) is unchanged — A/B cancel. The replacement sensitivity subset (tag C, §8.1) adds ≈ 50–65s. Net saving ≈ **198–213s of pytest / 195–215 whole-job runner-seconds, upper bound 263s, TBD pending a replacement-surface canary** (those minutes are charged on a third runner that currently runs a suite selection substantially overlapping 3.11's).
...
- **Net pytest saving (the A/B-cancel equation):** the conceptual change is
  - BEFORE: A targeted (0.87s) + B frozen (35.10s) + FULL (263.36s)
  - AFTER: A targeted (0.87s) + B frozen (35.10s) + C sensitivity subset (≈ 50–65s)
  - A and B therefore **cancel**: the standalone targeted/frozen prelude is unchanged, while the duplicate A/B executions that lived **inside** the removed FULL suite disappear with it. Net pytest saving = 263.36s (FULL) − C (≈ 50–65s) ≈ **198–213s**. A/B is subtracted **once** (as part of the FULL step's removal) and never again — the 0.87s + 35.10s prelude is not subtracted a second time.
- **Whole-job runner seconds (net):** portability whole job 327 → ≈ **116–131s** (unchanged job overhead + A 0.87 + B 35.10 + C ≈ 50–65) → net ≈ **195–215s**; **theoretical gross upper bound ≈ 263s** (achievable only if C cost zero, which it does not). **TBD pending a replacement-surface canary.**
