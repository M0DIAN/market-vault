"""L4-owned read-only Windows object, volume and access evidence; not qualification."""
...
def _native_volume_root(handle):
    ...
    _require(_drive_type(mount) == 3 and guid.value.startswith("\\\\?\\Volume{")
             ...
             "UNSAFE_PATH", "nonlocal or mismatched volume-root evidence")
...
def _ntfs_capability(filesystem):
    ...
    try:
        ...
        _require(data.serial == serial and extended_size >= 8 and major > 0,
                 "UNSAFE_PATH", "NTFS format/volume identity differs")
...
class _WindowsObject:
    ...
    def _facts(self):
        ...
        _require(identity.serial != 0 and any(identity.identifier), "PLATFORM_UNQUALIFIED", "unstable FileIdInfo")
        _require(standard.size >= 0, "UNSAFE_PATH", "negative native file size")
        ...
        _ok(_volume_name(mount, guid, len(guid)))
        _require(_drive_type(mount) == 3 and guid_path.startswith(guid.value), "UNSAFE_PATH", "nonlocal or mismatched volume")
    ...
    def read_bytes(self, size):
        _require(not self.directory and self.handle is not None, "UNSAFE_PATH", "held file required")
        ...
        while offset < size:
            ...
            position = _Overlapped(0, 0, offset & 0xffffffff, offset >> 32, None)
            # Synchronous ReadFile with explicit offset; no SetFile* API is bound.
            _ok(_read(self.handle, buffer, length, c.byref(count), c.byref(position)))
            _require(0 < count.value <= length, "PHYSICAL_DRIFT", "short native read")
