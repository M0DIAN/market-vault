"""Built-in rolling_volume_mean Feature transform (v0.5.0 PR-3).

rolling_volume_mean is the arithmetic mean of the trailing ``window_bars``
volume values the executor selected:

``rolling_volume_mean = math.fsum(volume_values) / window_bars``
...
def rolling_volume_mean(input_: FeatureTransformInput) -> float:
    """Arithmetic mean of the trailing ``window_bars`` volume values.
    ...
    if not math.isfinite(result):
        raise ValueError("rolling_volume_mean result must be finite")
    return result
...
def _window_bars(input_: FeatureTransformInput) -> int:
    ...
    raise ValueError("rolling_volume_mean requires the window_bars parameter")


def _require_row_count(input_: FeatureTransformInput, window_bars: int) -> None:
    if len(input_.rows) != window_bars:
        raise ValueError(
            f"rolling_volume_mean consumes exactly window_bars rows, got "
            f"{len(input_.rows)}"
        )
