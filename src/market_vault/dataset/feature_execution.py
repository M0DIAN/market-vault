...
The PIT-to-Canonical binding and provenance verification shared with the
Label executor (PR-4) lives in the private
:mod:`market_vault.dataset.execution_provenance` module; the Feature
executor reuses it and converts its private errors to
:class:`FeatureExecutionError` at this public boundary.
"""
...
def execute_builtin_features(builds, pit_result, feature_specs) -> FeatureExecutionResult:
    ...
    """
    try:
        return _execute_builtin_features(builds, pit_result, feature_specs)
    except ExecutionProvenanceError as exc:
        raise FeatureExecutionError(str(exc)) from exc


def _execute_builtin_features(builds, pit_result, feature_specs) -> FeatureExecutionResult:
    try:
...
def _trailing_window(
    ...
    versions: tuple[str, ...],
) -> tuple[list[ResolvedRow], tuple[str, ...], str | None]:
