"""The five frozen logical identity domains; physical metadata never enters."""
...
from ._validation import checked_record, digest, require, stage
from .models import (
    MultiSourceCrossDayDatasetIdentityInput, MultiSourceCrossDaySampleAudit,
    MultiSourceCrossDayCompletionEntry, MultiSourceCrossDayCompletionSummary,
)

MULTI_SOURCE_CROSS_DAY_DATASET_ID_VERSION = "multi-source-cross-day-dataset-id-v1"
MULTI_SOURCE_CROSS_DAY_DATASET_ORCHESTRATION_CONTRACT_VERSION = "multi-source-cross-day-dataset-orchestration-v1"
MULTI_SOURCE_CROSS_DAY_SAMPLE_AUDIT_VERSION = "multi-source-cross-day-sample-audit-v1"
SEQUENCE_ROLES = frozenset((
    "CANONICAL_BUILD_PINS", "CANONICAL_ROW_VERSIONS", "COMPLETION_ENTRIES", "CROSS_DAY_DECISIONS",
    "CROSS_DAY_IMPLEMENTATION_PINS", "CROSS_DAY_SAMPLE_BINDINGS", "CROSS_DAY_SPEC_PINS", "GAP_REFERENCES",
    "MULTI_SOURCE_SAMPLE_VERSIONS", "OBSERVATION_BUILDPIN_IDS", "OBSERVATION_COVERAGE_IDS",
    "OBSERVATION_IMPLEMENTATION_PINS", "OBSERVATION_INPUT_PROOFS", "OBSERVATION_SPEC_PINS", "SAMPLE_AUDIT",
    "TS2_REGISTRY_PINS", "TS2_SPEC_PINS",
))
ORDERED_ROLES = frozenset(("COMPLETION_ENTRIES", "CROSS_DAY_DECISIONS", "CROSS_DAY_SAMPLE_BINDINGS",
                           "MULTI_SOURCE_SAMPLE_VERSIONS", "SAMPLE_AUDIT"))


def cross_day_dataset_sequence_id(role: str, members: tuple[str, ...]) -> str:
    require(type(role) is str and role in SEQUENCE_ROLES and type(members) is tuple,
            "IDENTITY_AUTHORITY", "closed sequence role/tuple required")
    members = tuple(digest(v) for v in members)
    if role not in ORDERED_ROLES:
        members = tuple(sorted(set(members)))
    return encode_identity("multi-source-cross-day-sequence-v1",
                           {"role": role, "count": len(members), "members": "".join(members)})


def multi_source_cross_day_sample_audit_id(audit: MultiSourceCrossDaySampleAudit) -> str:
    with stage("IDENTITY_AUTHORITY"):
        checked_record(audit, MultiSourceCrossDaySampleAudit, "IDENTITY_AUTHORITY")
        return encode_identity(MULTI_SOURCE_CROSS_DAY_SAMPLE_AUDIT_VERSION,
                               {f.name: getattr(audit, f.name) for f in fields(audit)})


def multi_source_cross_day_completion_entry_id(entry: MultiSourceCrossDayCompletionEntry) -> str:
    checked_record(entry, MultiSourceCrossDayCompletionEntry, "IDENTITY_AUTHORITY")
    return encode_identity("multi-source-cross-day-completion-entry-v1",
        dict(code=entry.code, trade_date=entry.trade_date, status=entry.status, reason_code=entry.reason_code))


def multi_source_cross_day_completion_content_id(summary: MultiSourceCrossDayCompletionSummary) -> str:
    checked_record(summary, MultiSourceCrossDayCompletionSummary, "IDENTITY_AUTHORITY")
    return encode_identity("multi-source-cross-day-completion-v1", dict(
        complete_count=summary.complete_count, incomplete_count=summary.incomplete_count, missing_count=summary.missing_count,
        entries_digest=cross_day_dataset_sequence_id("COMPLETION_ENTRIES",
            tuple(multi_source_cross_day_completion_entry_id(e) for e in summary.entries))))
...
def _payload(value):
    ...
    # Closed section-17 payload: exactly 49 explicitly named scalar fields.
    return dict(
        ...
        observation_values_content_id=observation_feature_values_content_id(obs),
...
def multi_source_cross_day_dataset_id(value: MultiSourceCrossDayDatasetIdentityInput) -> str:
    """Validate the complete recorded declaration; calculation cannot issue a result."""
    from .execution import _validate_identity_input
    with stage("IDENTITY_AUTHORITY"):
        _validate_identity_input(value)
        return encode_identity(MULTI_SOURCE_CROSS_DAY_DATASET_ID_VERSION, _payload(value))
