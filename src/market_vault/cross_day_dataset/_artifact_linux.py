"""Linux retained descriptor/statx evidence; no native publication call here."""
...
def _ext4_capability(fd):
    ...
    _require(_mount_id(fd) == before, "FILESYSTEM_MISMATCH", "mount identity drift")
    return result
...
class _LinuxObject:
    ...
    def _facts(self):
        _require(self.fd is not None, "OWNERSHIP_UNPROVEN", "closed descriptor")
        ...
        return (data.st_dev, data.st_ino, self.directory), _filesystem(self.fd), security

    def recheck(self):
        _require(self._facts() == (self.identity, self.filesystem, self.security), "UNSAFE_PATH", "held Linux object drift")
    ...
    def read_bytes(self):
        _require(not self.directory and self.fd is not None, "UNSAFE_PATH", "regular held file required")
